...
logger = logging.getLogger("interviewprep.incremental_dag")
...
def _get_batch_id(**kwargs) -> str:
    ...
    return f"{execution_date.strftime('%Y-%m-%d')}_weekly"
...
def _demo_settings(**kwargs) -> dict:
    ...
    return {
        ...
        "leetcode_limit": _as_int(
            run_conf.get("demo_limit_leetcode", _safe_variable_get("demo_limit_leetcode", "5")), 5,
        ),
        "gfg_limit": _as_int(
            run_conf.get("demo_limit_gfg", _safe_variable_get("demo_limit_gfg", "5")), 5,
        ),
        "gfg_max_sitemaps": _as_int(
            run_conf.get("demo_limit_gfg_sitemaps", _safe_variable_get("demo_limit_gfg_sitemaps", "1")), 1,
        ),
        "medium_limit": _as_int(
            run_conf.get("demo_limit_medium", _safe_variable_get("demo_limit_medium", "5")), 5,
        ),
...
def scrape_geeksforgeeks(**kwargs):
    print("=" * 60)
    print("Starting GFG scraper - INCREMENTAL MODE")
    print("=" * 60)
    ...
    scraper = GFGScraper(scrape_type='incremental', config=config, storage=storage)
...
def scrape_leetcode(**kwargs):
    print("=" * 60)
    print("Starting LeetCode scraper - INCREMENTAL MODE")
    print("=" * 60)
    demo = _demo_settings(**kwargs)
    storage = GCSBackend(bucket_name=GCS_BUCKET_NAME, project_id=GCP_PROJECT_ID)
    ...
    if demo["enabled"]:
        config.INCREMENTAL_MAX_POSTS = max(0, demo["leetcode_limit"])
        print(f"Demo mode ON — limiting LeetCode INCREMENTAL_MAX_POSTS to {config.INCREMENTAL_MAX_POSTS}")

    scraper = LeetCodeScraper(scrape_type='incremental', config=config, storage=storage, fetch_comments=False)
    scraper.run()
...
def scrape_medium(**kwargs):
    print("=" * 60)
    print("Starting Medium scraper - INCREMENTAL MODE")
    print("=" * 60)
    demo = _demo_settings(**kwargs)
    storage = GCSBackend(bucket_name=GCS_BUCKET_NAME, project_id=GCP_PROJECT_ID)
    ...
    scraper = MediumScraper(scrape_type='incremental', config=config, storage=storage, log_dir='/tmp/medium_logs')
...
def print_summary(**kwargs):
    ...
    print("\n" + "=" * 60)
    print("INCREMENTAL SCRAPING PIPELINE SUMMARY")
    print("=" * 60)
...
def run_preprocessing(**kwargs):
    """Run preprocessing pipeline on incremental scraped data. Uses resume on retry."""
...
def build_email_body(**kwargs):
    ...
    body = f"""
    <h2>InterviewPrep Incremental Pipeline Report — {status}</h2>
    <p><b>Run Date:</b> {kwargs.get('logical_date', 'N/A')}</p>

    <h3>Scraping Results (Incremental)</h3>
    ...
    body += "<p>— InterviewPrep Airflow Incremental Pipeline</p>"

    ti.xcom_push(key="email_subject", value=f"[InterviewPrep] Incremental Pipeline {status} — {kwargs.get('logical_date', '')}")
...
dag = DAG(
    'incremental_scraping_pipeline',
    default_args=default_args,
    description='Incremental scrape interview experiences, preprocess, validate, and load to database',
    schedule='@weekly',
    catchup=False,
    tags=['scraping', 'preprocessing', 'database', 'incremental'],
)

start = BashOperator(
    task_id='start',
    bash_command='echo "Starting INCREMENTAL scraping at $(date)"',
    dag=dag,
)
...
scrape_leetcode_task = PythonOperator(
...
scrape_medium_task = PythonOperator(
